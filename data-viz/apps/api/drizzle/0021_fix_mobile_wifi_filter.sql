-- =============================================================================
-- Migration 0021: fix filtro is_wifi_connected em mv_score_ookla_mobile
--
-- Problema: na 0019, video_base e web_base da mv_score_ookla_mobile não
-- filtravam por is_wifi_connected. A perna speed (vw_qoe_unified) já filtrava
-- por network_type='mobile', mas video/web traziam tudo (inclusive WiFi/fibra).
-- Resultado: 422k rows mas só 11k (2.7%) tinham score_download/upload populado
-- — overlap minúsculo entre os ~17k tests Ookla mobile e os ~400k de video/web
-- com qualquer tipo de conexão.
--
-- Fix: adicionar is_wifi_connected IS NOT TRUE em video_base e web_base.
-- mv_score_ookla_fibra já filtrava is_wifi_connected = TRUE, sem problema.
--
-- Operação: DROP+CREATE WITH NO DATA (instantâneo) + REFRESH MATERIALIZED
-- VIEW por fora da migration para popular.
-- =============================================================================
...
-- ── FIX: video_base com is_wifi_connected IS NOT TRUE (mobile) ──
video_base AS (
    SELECT attr_geohash7 AS gh,
        CASE WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%CLARO%' THEN 'CLARO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%TIM%' THEN 'TIM' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%VIVO%' THEN 'VIVO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%OI%' THEN 'OI' ELSE 'OUTROS' END AS op,
        DATE_TRUNC('month', ts_result)::DATE AS pm,
...
-- ── FIX: web_base com is_wifi_connected IS NOT TRUE (mobile) ──
web_base AS (
    SELECT attr_geohash7 AS gh,
        CASE WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%CLARO%' THEN 'CLARO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%TIM%' THEN 'TIM' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%VIVO%' THEN 'VIVO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%OI%' THEN 'OI' ELSE 'OUTROS' END AS op,
        DATE_TRUNC('month', ts_result)::DATE AS pm,
...
COMMENT ON MATERIALIZED VIEW mv_score_ookla_mobile IS
    'Snapshot diário do método Ookla logístico (mobile). v2: video/web filtrados por is_wifi_connected IS NOT TRUE (fix da 0019). Criada WITH NO DATA — popular via REFRESH MATERIALIZED VIEW.';--> statement-breakpoint


-- ---------------------------------------------------------------------------
-- Recriar vw_score_comparativo_mobile (dropada pelo CASCADE acima)
-- ---------------------------------------------------------------------------
