-- =============================================================================
-- Migration 0010: Compute real trend_delta from share variation
--
-- Problem: trend_direction and trend_delta were hardcoded as 'STABLE' / 0.0
--   since the initial view creation (stub waiting for historical data).
--
-- Fix: Add a second LATERAL JOIN on share_raw to fetch the previous month's
--   share_pct, then compute:
--     trend_delta = current_share - previous_share  (in percentage points)
--     trend_direction = UP (>1pp) | DOWN (<-1pp) | STABLE
-- =============================================================================
...
base AS (
    SELECT
        ...
        -- has_vivo_data: TRUE only when Vivo has real measurement samples
        (sp.vivo_score IS NOT NULL)               AS has_vivo_data,
        ...
        -- Trend: delta between current and previous period share
        COALESCE(sh.share_pct, 0) - COALESCE(sh_prev.share_pct, 0) AS trend_delta_raw,
        CASE
    ...
    -- Previous period share for trend calculation
    LEFT JOIN LATERAL (
        SELECT share_pct
        FROM share_raw sr
        WHERE sr.geohash_id = gc.geohash_id
          AND sr.precision = gc.precision
          AND sr.period_month < sh.period_month
        ORDER BY sr.period_month DESC
        LIMIT 1
    ) sh_prev ON sh.period_month IS NOT NULL
...
SELECT
    ...
    CASE
        WHEN trend_delta_raw >  1.0 THEN 'UP'::trend_direction
        WHEN trend_delta_raw < -1.0 THEN 'DOWN'::trend_direction
        ELSE 'STABLE'::trend_direction
    END AS trend_direction,
    ROUND(trend_delta_raw::NUMERIC, 2) AS trend_delta
FROM base
...
CREATE INDEX IF NOT EXISTS idx_mvw_gh_summary_no_vivo
