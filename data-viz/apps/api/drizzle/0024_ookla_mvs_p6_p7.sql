-- =============================================================================
-- Migration 0024: mv_score_ookla_mobile/fibra com UNION ALL p6+p7
--
-- Ate 0022 as MVs Ookla so geravam scores em precision=7. Padrao das
-- mv_score_mobile/fibra (Vivo) ja era ter p6+p7 via UNION ALL nas views fonte.
--
-- Mudancas:
--   1. Cada CTE base (speed/video/web) ganha "raw" intermediaria + UNION ALL
--      gerando linhas para p7 e p6.
--   2. Agregadores (speed_percentis, video_agg, web_agg) recebem `prec` no
--      SELECT e GROUP BY.
--   3. JOINs no CTE `joined` casam por (gh, prec, op, pm).
--   4. SELECT final usa j.prec ao inves de 7::SMALLINT hardcoded.
--   5. vw_score_comparativo_mobile/fibra: WHERE v.precision = 7 -> 6.
--
-- Performance: REFRESH inicial agora processa ~1.5x mais dados (gh6 tem
-- menor cardinalidade, ~25% mais combinacoes do que so gh7).
-- =============================================================================
...
WITH
speed_base_raw AS (
    SELECT attr_geohash7, attr_geohash6,
...
speed_base AS (
    SELECT attr_geohash7 AS gh, 7::SMALLINT AS prec, op, pm, val_dl_throughput, val_ul_throughput, val_dl_latency_iqm, val_ul_latency_iqm
    FROM speed_base_raw WHERE attr_geohash7 IS NOT NULL
    UNION ALL
    SELECT attr_geohash6 AS gh, 6::SMALLINT AS prec, op, pm, val_dl_throughput, val_ul_throughput, val_dl_latency_iqm, val_ul_latency_iqm
    FROM speed_base_raw WHERE attr_geohash6 IS NOT NULL
),
speed_percentis AS (
    SELECT gh, prec, op, pm, COUNT(*)::INTEGER AS total_testes_speed,
    ...
    FROM speed_base GROUP BY gh, prec, op, pm
),
speed_params AS (
    SELECT
...
video_base AS (
    SELECT attr_geohash7 AS gh, 7::SMALLINT AS prec, op, pm, attr_video_rebuffering_count, val_video_time_to_start, razao_alta_resolucao
    FROM video_base_raw WHERE attr_geohash7 IS NOT NULL
    UNION ALL
    SELECT attr_geohash6 AS gh, 6::SMALLINT AS prec, op, pm, attr_video_rebuffering_count, val_video_time_to_start, razao_alta_resolucao
    FROM video_base_raw WHERE attr_geohash6 IS NOT NULL
),
video_agg AS (
    SELECT gh, prec, op, pm, COUNT(*)::INTEGER AS total_testes_video,
    ...
    FROM video_base GROUP BY gh, prec, op, pm
),
web_base_raw AS (
    SELECT attr_geohash7, attr_geohash6,
        CASE WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%CLARO%' THEN 'CLARO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%TIM%' THEN 'TIM' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%VIVO%' THEN 'VIVO' WHEN UPPER(TRIM(attr_sim_operator_common_name)) LIKE '%OI%' THEN 'OI' ELSE 'OUTROS' END AS op,
...
web_base AS (
    SELECT attr_geohash7 AS gh, 7::SMALLINT AS prec, op, pm, val_web_page_load_time FROM web_base_raw WHERE attr_geohash7 IS NOT NULL
    UNION ALL
    SELECT attr_geohash6 AS gh, 6::SMALLINT AS prec, op, pm, val_web_page_load_time FROM web_base_raw WHERE attr_geohash6 IS NOT NULL
),
web_agg AS (
    SELECT gh, prec, op, pm, COUNT(*)::INTEGER AS total_testes_web,
        AVG((val_web_page_load_time < 5000)::INT::NUMERIC) AS razao_carga_lt_5s
    FROM web_base GROUP BY gh, prec, op, pm
),
joined AS (
    SELECT
        COALESCE(s.gh, v.gh, w.gh) AS gh,
        COALESCE(s.prec, v.prec, w.prec) AS prec,
    ...
    FROM speed_scores s
    FULL OUTER JOIN video_agg v ON s.gh=v.gh AND s.prec=v.prec AND s.op=v.op AND s.pm=v.pm
    FULL OUTER JOIN web_agg w   ON COALESCE(s.gh,v.gh)=w.gh AND COALESCE(s.prec,v.prec)=w.prec AND COALESCE(s.op,v.op)=w.op AND COALESCE(s.pm,v.pm)=w.pm
)
SELECT
    j.gh AS geohash_id, j.prec AS precision, j.pm AS period_month, j.op AS operator,
    ...
    jsonb_build_object(
        'metodo', 'ookla_consolidado_v3_inline',
        'modo', 'mobile',
...
COMMENT ON MATERIALIZED VIEW mv_score_ookla_mobile IS
    'Snapshot diário do método Ookla logístico (mobile). v4: UNION ALL para precision 6 e 7 (alem do calc_trace enriquecido). WITH NO DATA — popular via REFRESH.';--> statement-breakpoint
...
COMMENT ON MATERIALIZED VIEW mv_score_ookla_fibra IS
    'Snapshot diário do método Ookla logístico (fixed/fibra). v4: UNION ALL para precision 6 e 7. WITH NO DATA — popular via REFRESH.';--> statement-breakpoint
...
WHERE v.precision = 6;--> statement-breakpoint
