-- =============================================================================
-- Migration 0022: restaura LEFT JOIN LATERAL para share em vw_geohash_summary
--
-- A 0006_share_latest_join introduziu LATERAL para pegar o share mais recente
-- onde share.period_month <= score.period_month, lidando com o fato de que
-- vw_share_real só tem 2025-12 enquanto scores cobrem 2025-12 a 2026-03.
-- A 0018 regrediu para LEFT JOIN com igualdade (sh.period_month = spc.period_month),
-- fazendo share_vivo = 0 em todos os meses ≠ 2025-12.
--
-- Fix: reaplicar o padrão LATERAL preservando todas as colunas adicionadas
-- por 0018/0020 (split por tecnologia + has_vivo_data).
-- =============================================================================
...
base AS (
    ...
    LEFT JOIN LATERAL (
        ...
        WHERE sr.geohash_id = gc.geohash_id
          ...
          AND sr.period_month <= COALESCE(spc.period_month, '9999-12-31'::date)
    ...
    WHERE spc.geohash_id IS NOT NULL OR sh.period_month IS NOT NULL
),
quadrants AS (
    SELECT
        b.*,
...
COMMENT ON MATERIALIZED VIEW vw_geohash_summary IS
    'v4.2 — restaura LATERAL JOIN p/ share (latest <= score period), corrigindo regressão da 0018 que zerava share_vivo em meses sem share. Mantém has_vivo_data e split por tecnologia.';--> statement-breakpoint
