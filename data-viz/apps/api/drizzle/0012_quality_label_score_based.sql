-- =============================================================================
-- Migration 0012: Quality label based on score ranges (0-100)
--
-- Problem: quality_label was computed from raw download/latency thresholds
--   (EXCELENTE: DL>=100 AND Lat<=20, etc.) which doesn't match the
--   stakeholder-defined criteria.
--
-- Fix: Use vivo_score (composite QoE 0-10, scaled to 0-100) to derive
--   quality_label using the stakeholder ranges:
--     - EXCELENTE: 80–100
--     - BOM:       60–80
--     - REGULAR:   40–60
--     - RUIM:      0–40
...
base AS (
    SELECT
        ...
        -- Quality label: based on vivo_score (0-10) scaled to 0-100
        CASE
            WHEN COALESCE(sp.vivo_score, 0) * 10 >= 80 THEN 'EXCELENTE'::quality_label
            WHEN COALESCE(sp.vivo_score, 0) * 10 >= 60 THEN 'BOM'::quality_label
            WHEN COALESCE(sp.vivo_score, 0) * 10 >= 40 THEN 'REGULAR'::quality_label
