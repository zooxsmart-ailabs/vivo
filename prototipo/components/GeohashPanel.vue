          ...
          <div style="background: #fff; border-radius: 10px; border: 1px solid rgba(0,0,0,0.07); padding: 10px 12px; box-shadow: 0 1px 4px rgba(0,0,0,0.05);">
            ...
            <div style="background: #FAFAFA; border-radius: 7px; border: 1px solid rgba(0,0,0,0.07); padding: 7px 10px; margin-bottom: 6px;">
              ...
              <p style="font-size: 10.5px; color: #3A3A3C; line-height: 1.5; margin: 0;">
                Área de renda média <strong>R$&nbsp;{{ g.demographics.avgIncome.toLocaleString('pt-BR') }}</strong>, com aproximadamente <strong>{{ (Math.round(g.demographics.populationDensity * 1.22 / 100) * 100).toLocaleString('pt-BR') }} habitantes</strong> em {{ g.demographics.populationDensity > 8000 ? 'alta' : g.demographics.populationDensity > 4000 ? 'média' : 'baixa' }} densidade e crescimento populacional <strong>{{ g.demographics.growthLabel.toLowerCase() }}</strong>.<br>
                <template v-if="hasMovel">Share Móvel de <strong>{{ g.marketShare.shareMovel || g.diagnostico.sharePenetracao }}%</strong>: {{ (g.marketShare.shareMovel || g.diagnostico.sharePenetracao) > 40 ? 'mercado saturado, priorizar retenção e upsell.' : (g.marketShare.shareMovel || g.diagnostico.sharePenetracao) > 25 ? 'penetração em zona de atenção, pressão competitiva crescente.' : 'baixa penetração, janela de expansão aberta.' }}</template><template v-if="hasFibra"><br>Share Fibra de <strong>{{ g.marketShare.shareFibra || 0 }}%</strong>: {{ (g.marketShare.shareFibra || 0) > 40 ? 'posição consolidada, monitorar risco de churn.' : (g.marketShare.shareFibra || 0) > 25 ? 'disputa acirrada com concorrentes, manter pressão comercial.' : 'mercado pouco explorado, alto potencial de captação.' }}</template>
              </p>
            </div>
            ...
            <div style="background: #FAFAFA; border-radius: 7px; border: 1px solid rgba(0,0,0,0.07); padding: 7px 10px; margin-bottom: 6px;">
              ...
              <p style="font-size: 10.5px; color: #3A3A3C; line-height: 1.5; margin: 0;">
                ...
                <template v-if="hasFibra">Projeção Fibra (3 meses): <strong :style="{ color: (g.shareTrend.deltaFibra || 0) > 0 ? '#15803D' : (g.shareTrend.deltaFibra || 0) < 0 ? '#DC2626' : '#8E8E93' }">{{ (g.shareTrend.deltaFibra || 0) > 0 ? '+' : '' }}{{ (g.shareTrend.deltaFibra || 0).toFixed(1) }}pp</strong> — {{ (g.shareTrend.deltaFibra || 0) > 0 ? 'tendência de crescimento, manter pressão comercial.' : (g.shareTrend.deltaFibra || 0) < 0 ? 'queda projetada, acionar defesa imediata.' : 'estável, monitorar movimentos da concorrência.' }}</template><template v-if="hasMovel"><br>Projeção Móvel (3 meses): <strong :style="{ color: (g.shareTrend.deltaMovel || g.shareTrend.delta || 0) > 0 ? '#15803D' : (g.shareTrend.deltaMovel || g.shareTrend.delta || 0) < 0 ? '#DC2626' : '#8E8E93' }">{{ (g.shareTrend.deltaMovel || g.shareTrend.delta || 0) > 0 ? '+' : '' }}{{ (g.shareTrend.deltaMovel || g.shareTrend.delta || 0).toFixed(1) }}pp</strong> — {{ (g.shareTrend.deltaMovel || g.shareTrend.delta || 0) > 0 ? 'tendência positiva, capitalizar com upgrade de plano.' : (g.shareTrend.deltaMovel || g.shareTrend.delta || 0) < 0 ? 'queda projetada, acionar retenção proativa.' : 'estável, priorizar qualidade de atendimento.' }}</template>
              </p>
            </div>
