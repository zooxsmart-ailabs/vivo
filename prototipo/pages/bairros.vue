...
<template>
  <div style="height:100%;display:flex;flex-direction:column;overflow:hidden;background:#F2F2F7;">
    ...
    <div style="background:white;border-bottom:1px solid rgba(0,0,0,0.07);padding:10px 16px;display:flex;align-items:center;gap:8px;flex-shrink:0;flex-wrap:wrap;">
      <span style="font-size:10px;font-weight:700;color:#8E8E93;letter-spacing:0.06em;text-transform:uppercase;margin-right:4px;white-space:nowrap;">Categoria</span>
      <button
      ...
      <div style="flex:1;min-width:160px;max-width:260px;position:relative;margin-left:8px;">
        ...
        <input
          ...
          style="width:100%;padding-left:28px;padding-right:10px;padding-top:6px;padding-bottom:6px;font-size:13px;border:1px solid rgba(0,0,0,0.1);border-radius:20px;outline:none;background:#F2F2F7;color:#1C1C1E;box-sizing:border-box;height:28px;font-family:inherit;"
        />
      </div>
      <span style="margin-left:auto;font-size:11px;color:#8E8E93;white-space:nowrap;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;">
      ...
      <aside style="width:260px;flex-shrink:0;border-right:1px solid rgba(0,0,0,0.07);background:white;overflow-y:auto;">
        <div v-if="rankingList.length===0" style="padding:32px;text-align:center;color:#8E8E93;">
          ...
          <p style="font-size:12px;">Nenhum bairro encontrado</p>
        </div>
        <button
        ...
        >
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ...
            <div style="flex:1;min-width:0;">
              <p style="font-size:12px;font-weight:700;color:#1C1C1E;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin:0;">{{ bairro.name }}</p>
            </div>
            <div style="text-align:right;flex-shrink:0;">
              <p :style="{fontSize:'14px',fontWeight:700,color:tabColor,margin:0}">{{ bairro.avgShare }}%</p>
              <p style="font-size:11px;color:#8E8E93;margin:0;">share</p>
            </div>
          </div>
      ...
      <main style="flex:1;overflow:hidden;display:flex;flex-direction:column;min-width:0;">
        ...
        <div v-if="!selectedBairro" style="height:100%;overflow-y:auto;padding:20px 24px;background:#F2F2F7;">
          <div style="margin-bottom:16px;">
            ...
            <p style="font-size:11px;color:#8E8E93;margin:3px 0 0;">
              ...
              <table style="width:100%;border-collapse:collapse;">
                <thead>
                  <tr style="background:#F9F9FB;border-bottom:1px solid rgba(0,0,0,0.07);">
                    <th v-for="h in ['Bairro','Geohash','Quadrante','Tecnologia','Share','Ookla','ARPU','Tendência','Score','Prioridade']" :key="h"
                      style="padding:8px 10px;text-align:left;font-size:10px;font-weight:700;color:#8E8E93;letter-spacing:0.06em;text-transform:uppercase;white-space:nowrap;">
                      {{ h }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(g, i) in sortedFrentes" :key="g.id"
                    ...
                    <td style="padding:6px 10px;font-size:12px;font-weight:600;color:#1C1C1E;white-space:nowrap;">{{ g.neighborhood }}</td>
                    <td style="padding:6px 10px;"><span style="font-family:monospace;font-size:11px;color:#8E8E93;">{{ g.id }}</span></td>
                    ...
                    <td style="padding:6px 10px;font-size:12px;color:#1C1C1E;">{{ g.crm?.arpuFibra ? `R$ ${g.crm.arpuFibra}` : "—" }}</td>
                    <td style="padding:6px 10px;">
                    ...
                    <td style="padding:6px 10px;"><span style="font-size:12px;font-weight:700;color:#660099;">{{ g.priorityScore.toFixed(1) }}</span></td>
                    <td style="padding:6px 10px;">
        ...
        <div v-else style="flex:1;overflow-y:auto;padding:18px;background:#F2F2F7;">
          ...
          <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;margin-bottom:12px;">
            <div style="display:flex;align-items:flex-start;justify-content:space-between;margin-bottom:14px;">
              <div>
                ...
                <p style="font-size:11px;color:#8E8E93;margin:0 0 8px;">
              ...
              <div style="text-align:right;flex-shrink:0;">
                ...
                <p style="font-size:11px;color:#8E8E93;margin:0;">Share Médio Vivo</p>
              </div>
            </div>
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:10px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">População</span>
                <span style="font-size:12px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.totalPopulation ? fmt(selectedBairro.totalPopulation) : "—" }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:10px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Clientes Vivo</span>
                <span style="font-size:12px;font-weight:700;color:#1C1C1E;">{{ fmt(selectedBairro.totalClients) }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:10px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Renda Média</span>
                <span style="font-size:12px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.avgIncome ? `R$ ${fmt(selectedBairro.avgIncome)}` : "—" }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:10px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Satisfação Vivo</span>
                <span style="font-size:12px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.avgVivoScore.toFixed(1) }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:10px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Delta vs. Concorrente</span>
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px;">
            ...
            <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;">
              <p style="font-size:12px;font-weight:700;color:#1C1C1E;margin:0 0 12px;">Satisfação SpeedTest (0–10)</p>
              <div
              ...
              >
                <span style="font-size:11px;font-weight:700;width:40px;flex-shrink:0;color:#1C1C1E;">{{ row.name }}</span>
            ...
            <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;">
              <div style="display:flex;align-items:center;margin-bottom:12px;">
                <p style="font-size:12px;font-weight:700;color:#1C1C1E;margin:0;">Distribuição Comercial</p>
                <span style="margin-left:auto;font-size:11px;color:#8E8E93;">{{ selectedBairro.geohashes.length }} geohashes</span>
              </div>
                ...
                <template v-for="q in QUADRANT_ORDER" :key="q">
                  <div v-if="(selectedBairro.quadrantCounts[q]??0) > 0">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                      ...
                      <span style="font-size:12px;font-weight:700;color:#1C1C1E;">
                        {{ selectedBairro.quadrantCounts[q] }}
                        <span style="font-size:11px;color:#8E8E93;font-weight:400;">({{ Math.round((selectedBairro.quadrantCounts[q]/selectedBairro.geohashes.length)*100) }}%)</span>
                      </span>
                    </div>
            ...
            <div v-if="detail!.ghsWithC2.length > 0" style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;">
              <div style="display:flex;align-items:center;margin-bottom:12px;">
                <p style="font-size:12px;font-weight:700;color:#1C1C1E;margin:0;">Infraestrutura</p>
                <span style="margin-left:auto;font-size:11px;color:#8E8E93;">{{ detail!.ghsWithC2.length }}/{{ selectedBairro.geohashes.length }} gh</span>
              </div>
              <div style="display:flex;flex-direction:column;gap:10px;">
                <div v-if="selectedBairro.hasFibra && detail!.avgFibraScore !== null">
                  <div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;">
                    ...
                    <span style="font-size:12px;font-weight:700;color:#1C1C1E;">Fibra</span>
                    <span style="margin-left:auto;font-size:12px;font-weight:700;color:#1C1C1E;">{{ detail!.avgFibraScore }}<span style="font-size:11px;color:#8E8E93;font-weight:400;">/100</span></span>
                  </div>
                  <div v-for="(cnt, cls) in detail!.fibraClassCounts" :key="cls" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px;">
                    <div style="display:flex;align-items:center;gap:5px;">
                      ...
                      <span style="font-size:12px;color:#3C3C43;">{{ FIBRA_LABELS[cls]??cls }}</span>
                    </div>
                ...
                <div v-if="selectedBairro.hasMovel && detail!.avgMovelScore !== null">
                  <div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;">
                    ...
                    <span style="font-size:12px;font-weight:700;color:#1C1C1E;">Móvel</span>
                    <span style="margin-left:auto;font-size:12px;font-weight:700;color:#1C1C1E;">{{ detail!.avgMovelScore }}<span style="font-size:11px;color:#8E8E93;font-weight:400;">/100</span></span>
                  </div>
                  <div v-for="(cnt, cls) in detail!.movelClassCounts" :key="cls" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px;">
                    <div style="display:flex;align-items:center;gap:5px;">
                      ...
                      <span style="font-size:12px;color:#3C3C43;">{{ MOVEL_LABELS[cls]??cls }}</span>
                    </div>
