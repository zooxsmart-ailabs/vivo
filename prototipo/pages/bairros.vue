        ...
        >
          <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
            ...
            <div style="text-align:right;flex-shrink:0;">
              <p :style="{fontSize:'13px',fontWeight:700,color:tabColor,margin:0}">{{ bairro.avgShare }}%</p>
              <p style="font-size:9px;color:#8E8E93;margin:0;">share</p>
            </div>
          </div>
          <div style="display:flex;align-items:center;gap:5px;margin-left:30px;flex-wrap:wrap;">
            <span :style="{fontSize:'9px',fontWeight:600,padding:'1px 6px',borderRadius:'20px',border:`1px solid ${tabColor}40`,color:tabColor}">
              {{ bairro.quadrantCounts[rankingTab]??0 }} gh
            </span>
            <span :style="{display:'inline-flex',alignItems:'center',gap:'2px',fontSize:'9px',fontWeight:600,color:trendColor(bairro.dominantTrend)}">
                ...
                <tbody>
                  <tr v-for="(g, i) in sortedFrentes" :key="g.id"
                    ...
                    <td style="padding:6px 10px;">
                      <span :style="{display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'10px',fontWeight:600,color:'#fff',background:QUADRANT_CONFIG[g.quadrant].color,padding:'3px 10px 3px 8px',borderRadius:'20px',whiteSpace:'nowrap'}">
                    ...
                    <td style="padding:6px 10px;">
                      <span :style="{display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'10px',fontWeight:600,padding:'3px 10px',borderRadius:'20px',whiteSpace:'nowrap',color:TECH_CONFIG[g.technology]?.color??'#660099',background:(TECH_CONFIG[g.technology]?.color??'#660099')+'15',border:`1px solid ${(TECH_CONFIG[g.technology]?.color??'#660099')}40`}">
                        {{ TECH_CONFIG[g.technology]?.label??g.technology }}
                      </span>
                    </td>
                    <td style="padding:6px 10px;">
                      <span :style="{fontSize:'11px',fontWeight:700,color:g.marketShare.percentage>=35?'#16a34a':'#d97706'}">{{ g.marketShare.percentage }}%</span>
                    </td>
                    <td style="padding:6px 10px;">
                      <span :style="{fontSize:'11px',fontWeight:700,color:g.diagnostico.scoreOokla>=7.5?'#16a34a':g.diagnostico.scoreOokla>=6.5?'#d97706':'#dc2626'}">{{ g.diagnostico.scoreOokla.toFixed(1) }}</span>
                    </td>
                    <td style="padding:6px 10px;font-size:11px;color:#1C1C1E;">{{ g.crm?.arpuFibra ? `R$ ${g.crm.arpuFibra}` : "—" }}</td>
                    <td style="padding:6px 10px;">
                      <span :style="{display:'inline-flex',alignItems:'center',gap:'3px',fontSize:'10px',fontWeight:600,color:trendColor(g.shareTrend?.direction??'STABLE')}">
                    ...
                    <td style="padding:6px 10px;">
                      <span :style="{fontSize:'10px',fontWeight:600,color:PRIORITY_CONFIG[g.priority]?.color??'#660099',border:`1px solid ${(PRIORITY_CONFIG[g.priority]?.color??'#660099')}40`,padding:'2px 7px',borderRadius:'20px'}">
          ...
          <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;margin-bottom:12px;">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:5px;">
                  <template v-for="(c, q) in selectedBairro.quadrantCounts" :key="q">
                    <span v-if="QUADRANT_CONFIG[q as Quadrant]"
                      :style="{display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'9px',fontWeight:700,padding:'2px 8px 2px 6px',borderRadius:'20px',background:QUADRANT_CONFIG[q as Quadrant].color,color:'#fff'}">
                  ...
                  <span :style="{display:'inline-flex',alignItems:'center',gap:'3px',fontSize:'9px',fontWeight:700,padding:'2px 6px',borderRadius:'20px',border:`1px solid ${trendColor(selectedBairro.dominantTrend)}40`,color:trendColor(selectedBairro.dominantTrend)}">
            ...
            <div style="display:flex;gap:8px;flex-wrap:wrap;">
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                ...
                <span style="font-size:14px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.totalPopulation ? fmt(selectedBairro.totalPopulation) : "—" }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:9px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Clientes Vivo</span>
                <span style="font-size:14px;font-weight:700;color:#1C1C1E;">{{ fmt(selectedBairro.totalClients) }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                <span style="font-size:9px;font-weight:600;color:#8E8E93;text-transform:uppercase;letter-spacing:0.05em;">Renda Média</span>
                <span style="font-size:14px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.avgIncome ? `R$ ${fmt(selectedBairro.avgIncome)}` : "—" }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
                ...
                <span style="font-size:14px;font-weight:700;color:#1C1C1E;">{{ selectedBairro.avgVivoScore.toFixed(1) }}</span>
              </div>
              <div style="background:#F9F9FB;border:1px solid rgba(0,0,0,0.06);border-radius:10px;padding:8px 12px;display:flex;flex-direction:column;gap:2px;min-width:90px;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px;">
            ...
            <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;">
              ...
              >
                ...
                <span :style="{fontSize:'11px',fontWeight:700,width:'28px',textAlign:'right',color:OPERATOR_COLORS[row.name]??'#660099'}">{{ row.score }}</span>
              </div>
            </div>
                  ...
                  <div v-if="(selectedBairro.quadrantCounts[q]??0) > 0">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
                      <div style="display:flex;align-items:center;gap:5px;">
                        ...
                        <span :style="{fontSize:'10px',fontWeight:600,color:QUADRANT_CONFIG[q].color}">{{ QUADRANT_LABELS[q] }}</span>
                      </div>
                      <span style="font-size:11px;font-weight:700;color:#1C1C1E;">
                        {{ selectedBairro.quadrantCounts[q] }}
                        <span style="font-size:9px;color:#8E8E93;font-weight:400;">({{ Math.round((selectedBairro.quadrantCounts[q]/selectedBairro.geohashes.length)*100) }}%)</span>
                      </span>
                    </div>
              ...
              <div style="display:flex;flex-direction:column;gap:10px;">
                <div v-if="selectedBairro.hasFibra && detail!.avgFibraScore !== null">
                  <div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;">
                    ...
                    <span style="margin-left:auto;font-size:13px;font-weight:700;color:#1C1C1E;">{{ detail!.avgFibraScore }}<span style="font-size:10px;color:#8E8E93;font-weight:400;">/100</span></span>
                  </div>
                  <div v-for="(cnt, cls) in detail!.fibraClassCounts" :key="cls" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px;">
                    <div style="display:flex;align-items:center;gap:5px;">
                      ...
                      <span style="font-size:10px;color:#3C3C43;">{{ FIBRA_LABELS[cls]??cls }}</span>
                    </div>
                    <span :style="{fontSize:'10px',fontWeight:700,color:FIBRA_COLORS[cls]??'#8E8E93'}">{{ cnt }}</span>
                  </div>
                </div>
                <div v-if="selectedBairro.hasMovel && detail!.avgMovelScore !== null">
                  <div style="display:flex;align-items:center;gap:5px;margin-bottom:6px;">
                    ...
                    <span style="margin-left:auto;font-size:13px;font-weight:700;color:#1C1C1E;">{{ detail!.avgMovelScore }}<span style="font-size:10px;color:#8E8E93;font-weight:400;">/100</span></span>
                  </div>
                  <div v-for="(cnt, cls) in detail!.movelClassCounts" :key="cls" style="display:flex;align-items:center;justify-content:space-between;margin-bottom:3px;">
                    <div style="display:flex;align-items:center;gap:5px;">
                      ...
                      <span style="font-size:10px;color:#3C3C43;">{{ MOVEL_LABELS[cls]??cls }}</span>
                    </div>
                    <span :style="{fontSize:'10px',fontWeight:700,color:MOVEL_COLORS[cls]??'#8E8E93'}">{{ cnt }}</span>
                  </div>
                </div>
              </div>
            </div>
