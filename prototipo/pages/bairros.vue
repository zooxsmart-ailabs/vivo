      ...
      <aside style="width:260px;flex-shrink:0;border-right:1px solid rgba(0,0,0,0.07);background:white;overflow-y:auto;">
        ...
        >
          ...
          <div style="display:flex;align-items:center;gap:5px;margin-left:30px;flex-wrap:wrap;">
            <span :style="{fontSize:'12px',fontWeight:600,padding:'1px 6px',borderRadius:'20px',border:`1px solid ${tabColor}40`,color:tabColor}">
              {{ bairro.quadrantCounts[rankingTab]??0 }} gh
            </span>
            <span :style="{display:'inline-flex',alignItems:'center',gap:'2px',fontSize:'12px',fontWeight:600,color:trendColor(bairro.dominantTrend)}">
              ...
              <table style="width:100%;border-collapse:collapse;">
                ...
                <tbody>
                  <tr v-for="(g, i) in sortedFrentes" :key="g.id"
                    ...
                    <td style="padding:6px 10px;">
                      <span :style="{display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'12px',fontWeight:600,padding:'3px 10px',borderRadius:'20px',whiteSpace:'nowrap',color:TECH_CONFIG[g.technology]?.color??'#660099',background:(TECH_CONFIG[g.technology]?.color??'#660099')+'15',border:`1px solid ${(TECH_CONFIG[g.technology]?.color??'#660099')}40`}">
                        {{ TECH_CONFIG[g.technology]?.label??g.technology }}
                      </span>
                    </td>
                    <td style="padding:6px 10px;">
                      <span :style="{fontSize:'12px',fontWeight:700,color:g.marketShare.percentage>=35?'#16a34a':'#d97706'}">{{ g.marketShare.percentage }}%</span>
                    </td>
                    <td style="padding:6px 10px;">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:5px;margin-top:5px;">
                  <template v-for="(c, q) in selectedBairro.quadrantCounts" :key="q">
                    <span v-if="QUADRANT_CONFIG[q as Quadrant]"
                      :style="{display:'inline-flex',alignItems:'center',gap:'4px',fontSize:'12px',fontWeight:700,padding:'2px 8px 2px 6px',borderRadius:'20px',background:QUADRANT_CONFIG[q as Quadrant].color,color:'#fff'}">
                  ...
                  <span :style="{display:'inline-flex',alignItems:'center',gap:'3px',fontSize:'12px',fontWeight:700,padding:'2px 6px',borderRadius:'20px',border:`1px solid ${trendColor(selectedBairro.dominantTrend)}40`,color:trendColor(selectedBairro.dominantTrend)}">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px;margin-bottom:12px;">
            ...
            <div style="background:white;border-radius:14px;border:1px solid rgba(0,0,0,0.06);box-shadow:0 1px 4px rgba(0,0,0,0.04);padding:16px;">
              ...
              >
                ...
                <span :style="{fontSize:'12px',fontWeight:700,width:'28px',textAlign:'right',color:OPERATOR_COLORS[row.name]??'#660099'}">{{ row.score }}</span>
              </div>
            </div>
